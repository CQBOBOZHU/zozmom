...
//				String[] sp = info.replace("{", "").replace("}", "").replace("\"", "").replace("\\", "").split(",");
//				for (String str : sp) {
...
//					if(key.contains("gt")){
//						gt=value;
//					}else if(key.contains("challenge")){
//						challenge=value;
//					}else if(key.contains("success")){
//						success=Integer.parseInt(value);
//					}
//				}
				...
				Log.v("zozmom", gt + "");
				Log.v("zozmom", challenge + "");
				Log.v("zozmom", success + "");
				if (gt.length() == 32) {
					return getSuccess();
				}
	...
	private String readContentFromGet(String getURL) throws IOException {
		...
		HttpURLConnection readConnection = (HttpURLConnection) url
				.openConnection();
		mReadConnection = readConnection;

		cookieManager = new CookieManager();

		Map<String, List<String>> headerFields = readConnection
				.getHeaderFields();
	...
	public String submitPostData(Map<String, String> params, String encode) {
		...
		try {
			...
			HttpURLConnection submitConnection = (HttpURLConnection) url
					.openConnection();
			mSubmitConneciton = submitConnection;
			if (cookieManager.getCookieStore().getCookies().size() > 0) {
				submitConnection.setRequestProperty("Cookie", TextUtils.join(
